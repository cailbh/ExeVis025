...
import allProblems from "@/assets/json/problems.json";
import conceptLinks from "@/assets/json/createdRel.json";
import conTree from "@/assets/json/conceptTree.json";

function generateConceptStats(problemIds) {
  let conTreeJson = deepClone(conTree);
  // 创建数据索引
  const problemMap = new Map(allProblems.map(p => [p.id, p]));
  const conceptRelationMap = new Map();
  // 第一步：构建问题-概念映射
